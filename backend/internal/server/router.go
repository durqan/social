...
func registerMessageRoutes(router *gin.Engine, database *gorm.DB) {
	...
	messages.POST("/:messageId/link-preview/import-video", middleware.RequireVerifiedEmail(database), middleware.RateLimitMiddleware(30, time.Hour), handlers.ImportMessageLinkPreviewVideo(database))
