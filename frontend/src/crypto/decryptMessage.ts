...
import { base64ToBytes, bytesToArrayBuffer, bytesToUtf8 } from "@/crypto/encoding.js";
...
export async function decryptMessage(message: Message, currentUserId: number, bundle: LocalE2EEKeyBundle): Promise<string> {
    ...
    const plaintext = await crypto.subtle.decrypt(
        {
            name: 'AES-GCM',
            iv: bytesToArrayBuffer(base64ToBytes(message.nonce || '')),
            additionalData: bytesToArrayBuffer(messageAAD(message.from_id, message.to_id)),
        },
        messageKey,
