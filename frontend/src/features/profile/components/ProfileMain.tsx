        ...
        <div className="mx-auto max-w-2xl">
            <div className="app-card overflow-hidden">
                <div className="relative">
                    ...
                    <div className="absolute -bottom-14 left-1/2 -translate-x-1/2 sm:-bottom-[76px] sm:left-6 sm:translate-x-0">
                        <div className="h-28 w-28 rounded-full bg-white p-1 sm:h-42 sm:w-42">
                            <button
                                type="button"
                                onClick={() => user?.avatar && setAvatarViewerOpen(true)}
                                disabled={!user?.avatar}
                                className="block h-full w-full rounded-full cursor-pointer disabled:cursor-default"
                            ...
                            >
                                <Avatar
                                    ...
                                    size="xl"
                                    className="h-full w-full text-xl sm:text-3xl"
                                />
                            </button>
                        </div>
                    </div>
                ...
                <div className="px-4 pb-5 pt-20 sm:px-6 sm:pb-6 sm:pt-[92px]">
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                        <div className="min-w-0 text-center sm:text-left">
                            <h1 className="break-words text-2xl font-bold text-gray-800">
                                {user?.name || 'Пользователь'}
                                {online && <span className="ml-2 text-green-500">●</span>}
                            </h1>
                            ...
                            {!isOwner && friendStatus === 'accepted' && currentUser && (
                                <button
                                    ...
                                    className="mt-4 w-full rounded-xl bg-sky-600 px-4 py-2 text-sm
                                    text-white cursor-pointer transition hover:bg-sky-700 sm:w-auto"
                                >
                                    Написать сообщение
                                </button>
                            )}
                        ...
                        <div className="flex gap-2 sm:flex-shrink-0">
                            {!isOwner && (
                                <button
                                    ...
                                    className={`w-full rounded-lg px-4 py-2 text-sm transition sm:w-auto ${
