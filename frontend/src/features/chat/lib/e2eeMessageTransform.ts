...
import { decryptMessage, isEncryptedMessage } from "@/crypto/decryptMessage.js";
import { decryptAttachmentForDisplay, isEncryptedAttachment } from "@/crypto/attachment.js";
...
export async function decryptMessagesForDisplay(messages: Message[], currentUserId?: number, bundle?: LocalE2EEKeyBundle | null): Promise<Message[]> {
    if (!currentUserId || !bundle) {
        return messages.map(message => markEncryptedUnreadable(message));
    }

    return Promise.all(messages.map(message => decryptMessageForDisplay(message, currentUserId, bundle)));
}

export async function decryptMessageForDisplay(message: Message, currentUserId: number, bundle: LocalE2EEKeyBundle): Promise<Message> {
    const next: Message = { ...message };

    if (next.reply_to_message) {
        next.reply_to_message = await decryptMessageForDisplay(next.reply_to_message, currentUserId, bundle);
    }
    if (next.forwarded_from_message) {
        next.forwarded_from_message = await decryptMessageForDisplay(next.forwarded_from_message, currentUserId, bundle);
    }
    if (next.attachments?.length) {
        next.attachments = await Promise.all(next.attachments.map(async attachment => {
